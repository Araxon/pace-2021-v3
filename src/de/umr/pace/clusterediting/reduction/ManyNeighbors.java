...
public class ManyNeighbors extends ReductionRule {
    ...
    protected Stack<Edge> execute(Graph graph, int k, boolean isInitialApplication) {
        ...
        if (result.size() != 0) {
            if (graph.p3Store.instanceNOTSolvableWithKGreedy(k - result.size())) {
                revert(result, graph);
                return null;
            }
            Stack<Edge> resultLastDepth = executeWithLastUsedDepth(graph, k - result.size());
            if (resultLastDepth == null) {
                lastUsedDepth.push(0);
                revert(result, graph);
                return null;
            }
            result.addAll(resultLastDepth);
        }
